// Package types provides unified type definitions for the docker-auto system.
// This package centralizes all type definitions to avoid duplication and ensure consistency.
package types
...
// Container represents a Docker container with detailed information
...
// ClientConfig represents Docker client configuration
type ClientConfig struct {
	Host               string                        `json:"host"`
	Version            string                        `json:"version"`
	APIVersion         string                        `json:"api_version"`
	TLSConfig          *TLSConfig                   `json:"tls_config,omitempty"`
	SecurityConfig     interface{}                  `json:"security_config,omitempty"`
	RegistryAuth       map[string]RegistryAuth      `json:"registry_auth,omitempty"`
	OperationTimeout   time.Duration                `json:"operation_timeout"`
	MaxConcurrentPulls int                          `json:"max_concurrent_pulls"`
	Timeout            time.Duration                `json:"timeout"`
	RetryConfig        *RetryConfig                 `json:"retry_config,omitempty"`
	Retry              *RetryConfig                 `json:"retry,omitempty"`
}

// TLSConfig represents TLS configuration for Docker client
...
// RetryConfig represents retry configuration for Docker operations
...
// HealthStatus represents the health status of a Docker service
...
type ContainerMetrics struct {
	...
	BlockRead         int64             `json:"block_read"`         // Bytes
	BlockWrite        int64             `json:"block_write"`        // Bytes
	PIDs              int               `json:"pids"`               // Number of processes
	Timestamp         time.Time         `json:"timestamp"`
	Labels            map[string]string `json:"labels,omitempty"`
}

// Lock provides thread-safe access to container metrics
func (cm *ContainerMetrics) Lock() {
	cm.mu.Lock()
}

// Unlock releases the lock on container metrics
func (cm *ContainerMetrics) Unlock() {
	cm.mu.Unlock()
}

// RLock provides thread-safe read access to container metrics
func (cm *ContainerMetrics) RLock() {
	cm.mu.RLock()
}

// RUnlock releases the read lock on container metrics
func (cm *ContainerMetrics) RUnlock() {
	cm.mu.RUnlock()
}
...
// Additional types for compatibility with docker module
...
type PullProgress struct {
	...
	Status         string                     `json:"status"`
	Progress       map[string]*LayerProgress  `json:"progress"`
...
// LayerProgress represents progress for a single layer
...
// ImageRemovalResult represents result of image removal operation
...
// ContainerUpdateResult represents result of container update operation
...
// ContainerUpdateOptions represents options for container updates
...
// BackupConfig represents backup configuration for updates
type BackupConfig struct {
	Enabled         bool   `json:"enabled"`
	BackupImage     string `json:"backup_image"`
	BackupContainer string `json:"backup_container"`
}
...
// ServiceHealthStatus represents the detailed health status of a Docker service
