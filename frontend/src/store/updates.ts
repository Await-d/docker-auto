...
export const useUpdatesStore = defineStore("updates", () => {
  ...
  const sortConfig = ref<UpdateSort>({
  ...
  const criticalUpdates = computed(() =>
    availableUpdates.value.filter(
      (u) => u.riskLevel === "high" || u.priority === "critical",
    ),
  );
  ...
  const scheduledUpdatesCount = computed(() => scheduledUpdates.value.length);

  const runningUpdatesCount = computed(() => runningUpdates.value.length);
  ...
  const filteredUpdates = computed(() => {
    ...
    if (filters.value.updateType?.length) {
      result = result.filter((u) =>
        filters.value.updateType!.includes(u.updateType),
      );
    }

    if (filters.value.riskLevel?.length) {
      result = result.filter((u) =>
        filters.value.riskLevel!.includes(u.riskLevel),
      );
    }
  ...
  const recentHistory = computed(() => updateHistory.value.slice(0, 10));
  ...
  async function fetchUpdateHistory(page?: number, filters?: UpdateFilter) {
    ...
    try {
      ...
      const recentHistory = response.items.filter(
        (item) => new Date(item.startedAt) > last30Days,
      );
      ...
      if (recentHistory.length > 0) {
        updateAnalytics.value.successRate =
          (recentHistory.filter((item) => item.status === "completed").length /
            recentHistory.length) *
          100;

        const completedUpdates = recentHistory.filter(
          (item) => item.status === "completed" && item.duration,
        );
  ...
  async function startUpdate(
    ...
    try {
      const result = await updatesAPI.startUpdate(updateId, {
        ...
        rollbackOnFailure:
          options?.rollbackOnFailure ?? updateSettings.value.rollbackOnFailure,
        notifyOnCompletion:
          options?.notifyOnCompletion ??
          updateSettings.value.enableNotifications,
  ...
  async function startBulkUpdate(
    ...
    const operation: BulkUpdateOperation = {
      ...
      maxConcurrent:
        options?.maxConcurrent || updateSettings.value.maxConcurrentUpdates,
    ...
    try {
      ...
      // Refresh data
      await Promise.all([checkForUpdates(), fetchRunningUpdates()]);
  ...
  async function cancelUpdate(runningUpdateId: string) {
    const runningUpdate = runningUpdates.value.find(
      (u) => u.id === runningUpdateId,
    );
    ...
    try {
      ...
      const index = runningUpdates.value.findIndex(
        (u) => u.id === runningUpdateId,
      );
      if (index !== -1) {
  ...
  async function unignoreUpdate(updateId: string) {
    ...
    try {
      ...
      ElMessage.success(
        `Update for "${update.containerName}" is no longer ignored`,
      );
    } catch (error) {
  ...
  async function exportUpdateHistory(
  ...
  function handleUpdateAvailable(data: any) {
    ...
    const existing = availableUpdates.value.find(
      (u) => u.containerId === data.containerId,
    );
    if (existing) {
      // Update existing
