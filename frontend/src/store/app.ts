...
export const useAppStore = defineStore('app', () => {
  ...
  const setTheme = (newTheme: 'light' | 'dark' | 'auto') => {
    ...
    if (newTheme === THEMES.DARK || (newTheme === THEMES.AUTO && isDarkMode.value)) {
...
export const useApp = () => {
  ...
  return {
    ...
    showSuccess: (message: string, title = 'Success') =>
      appStore.addNotification({ type: 'success', title, message, duration: 3000 }),

    showError: (message: string, title = 'Error') =>
      appStore.addNotification({ type: 'error', title, message, duration: 5000 }),

    showWarning: (message: string, title = 'Warning') =>
      appStore.addNotification({ type: 'warning', title, message, duration: 4000 }),

    showInfo: (message: string, title = 'Info') =>
      appStore.addNotification({ type: 'info', title, message, duration: 3000 }),

    // Theme helpers
    toggleTheme: () => {
      const current = appStore.theme
      const next = current === THEMES.LIGHT ? THEMES.DARK :
                   current === THEMES.DARK ? THEMES.AUTO : THEMES.LIGHT
