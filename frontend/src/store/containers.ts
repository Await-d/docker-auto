...
export const useContainerStore = defineStore("containers", () => {
  ...
  async function updateContainerImage(id: string, options?: any) {
    ...
    try {
      ...
      const updateIndex = availableUpdates.value.findIndex(
        (u) => u.container === id,
      );
      if (updateIndex !== -1) {
  ...
  async function fetchHistoricalStats(
    ...
    try {
      const historical = await containerAPI.getHistoricalStats(
        id,
        period,
        interval,
      );
  ...
  async function createFromTemplate(
    ...
    try {
      const container = await containerAPI.createFromTemplate(
        templateId,
        overrides,
      );
      containers.value.unshift(container);
  ...
  function setSorting(
    field: ContainerSort["field"],
    direction?: ContainerSort["direction"],
  ) {
