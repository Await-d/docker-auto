...
export interface DockerSettings {
  ...
  defaultVolumeMounts?: Array<{
    source: string;
    target: string;
    type?: string;
  }>;
  autoRemoveDanglingImages?: boolean;
  imageVerification?: boolean;
  registryTimeout?: number;
  logDriver?: string;
  enableBuildKit?: boolean;
  dockerCliOptions?: Record<string, string>;
}
...
export const useSettingsStore = defineStore("settings", () => {
  ...
  function isDirtySection(section: string): boolean {
    return Array.from(dirtyFields.value).some((field) =>
      field.startsWith(`${section}.`),
    );
  }

  function isValidSection(section: string): boolean {
    return !Object.keys(validationErrors.value).some((field) =>
      field.startsWith(`${section}.`),
    );
  }
  ...
  function hasChangesInSection(section: string): boolean {
    ...
    return (
      JSON.stringify(settings.value[section as keyof SystemSettings]) !==
  ...
  const saveSettings = async (section?: keyof SystemSettings) => {
    ...
    try {
      ...
      await http.put("/api/system/config", payload);

      // Update original settings to reflect saved state
      if (section) {
        if (originalSettings.value) {
          originalSettings.value[section] = JSON.parse(
            JSON.stringify(settings.value[section]),
          );
        }
        // Clear dirty fields for this section
        const sectionFields = Array.from(dirtyFields.value).filter((field) =>
          field.startsWith(`${section}.`),
        );
    ...
    if (section) {
      if (settings.value) {
        settings.value[section] = JSON.parse(
          JSON.stringify(originalSettings.value[section]),
        );
  ...
  const testConfiguration = async (
    ...
    try {
      const response = await http.post(
        `/api/system/config/test/${section}`,
        config,
      );
  ...
  const exportSettings = async (sections?: (keyof SystemSettings)[]) => {
    try {
      ...
      const response = await http.post("/api/system/config/export", payload, {
        responseType: "blob",
      });
  ...
  const importSettings = async (file: File) => {
    try {
      ...
      const response = await http.post("/api/system/config/import", formData, {
  ...
  const createSectionBackup = (section: keyof SystemSettings) => {
    if (settings.value) {
      sectionBackups.value[section] = JSON.parse(
        JSON.stringify(settings.value[section]),
      );
    }
  };
  ...
  const restoreSectionBackup = (section: keyof SystemSettings) => {
    if (sectionBackups.value[section] && settings.value) {
      settings.value[section] = JSON.parse(
        JSON.stringify(sectionBackups.value[section]),
      );
...
export const useSettings = () => {
  ...
  return {
    ...
    // Convenience methods
    getSectionSettings: <T extends keyof SystemSettings>(
      section: T,
    ): SystemSettings[T] | null => {
