...
export const useDashboardStore = defineStore('dashboard', () => {
  ...
  const createDefaultLayout = (): DashboardLayout => {
    const defaultWidgets = availableWidgets.value.slice(0, 6).map((def, index) => ({
      id: `widget-${Date.now()}-${index}`,
      type: def.type,
      title: def.name,
      component: def.component,
      position: { ...def.defaultPosition, w: def.defaultPosition.w || 3, h: def.defaultPosition.h || 3 } as WidgetPosition,
  ...
  const loadWidgetsForCurrentLayout = () => {
    ...
    state.activeWidgets = layout.widgets.filter(w => w.enabled).map(w => w.id)
  ...
  const createLayout = async (name: string, description?: string, copyFromId?: string) => {
  ...
  const updateLayout = async (layoutId: string, updates: Partial<DashboardLayout>) => {
  ...
  const deleteLayout = async (layoutId: string) => {
    ...
    if (state.currentLayoutId === layoutId) {
      const newCurrentLayout = state.layouts.find(l => l.id !== layoutId && l.isDefault) ||
  ...
  const addWidget = async (widgetType: string, position?: Partial<WidgetPosition>) => {
  ...
  const updateGlobalSettings = async (settings: Partial<typeof state.globalSettings>) => {
    Object.assign(state.globalSettings, settings)
    localStorage.setItem('dashboard-global-settings', JSON.stringify(state.globalSettings))
  }
