...
router.beforeEach(async (to, from, next) => {
  ...
  try {
    // Set page title
    const title = to.meta?.title ? `${to.meta.title} - Docker Auto Update System` : 'Docker Auto Update System'
    ...
    if (requiresAuth) {
      ...
      if (to.meta?.role) {
        if (!authStore.hasRole(to.meta.role as string)) {
          console.warn(`Access denied: Required role '${to.meta.role}' not found`)
          next('/403')
          return
        }
      }
      ...
      if (to.meta?.permission) {
        if (!authStore.hasPermission(to.meta.permission as string)) {
          console.warn(`Access denied: Required permission '${to.meta.permission}' not found`)
          next('/403')
          return
        }
      }
