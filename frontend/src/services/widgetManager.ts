...
import type { DashboardWidget, WidgetDefinition } from '@/store/dashboard'
...
class WidgetManagerService {
  ...
  private getWebSocketEvents(widgetType: string): string[] {
    const eventMap: Record<string, string[]> = {
      ...
      'update-activity': ['update:started', 'update:completed', 'update:failed'],
  ...
  private setupFocusRefresh(widget: DashboardWidget): void {
    ...
    // You might want to store cleanup functions for proper cleanup
  }
  ...
  private async fetchWidgetData(widget: DashboardWidget): Promise<any> {
    ...
    if (!response.ok) {
      throw new Error(`Failed to fetch data for widget ${widget.type}: ${response.statusText}`)
    }
   ...
   */
  private updateMetrics(widgetId: string, updates: Partial<WidgetMetrics>): void {
   ...
   */
  subscribeToEvents(eventType: string, callback: (event: WidgetEvent) => void): () => void {
    ...
    // For simplicity, we'll use a reactive effect
    // In a real implementation, you might want a proper event emitter
    const stopWatching = () => {
   ...
   */
  async refreshMultipleWidgets(widgetIds: string[], force = false): Promise<void> {
    const refreshPromises = widgetIds.map(id => this.refreshWidget(id, force))
   ...
   */
  getWidgetMetrics(widgetId?: string): WidgetMetrics | Map<string, WidgetMetrics> {
    if (widgetId) {
...
export function useWidgetManager() {
  return {
    ...
    refreshMultipleWidgets: widgetManager.refreshMultipleWidgets.bind(widgetManager)
  }
}
